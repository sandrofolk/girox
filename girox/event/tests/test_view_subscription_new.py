...
class SubscriptionNewGet(TestCase):
    def setUp(self):
        self.resp = self.client.get(r('events:subscription_new'))
...
class SubscriptionsNewPostValid(TestCase):
    def setUp(self):
        ...
        data = dict(name='Participante 1', rg='12345678901',
                    email='email@email.com', phone='(012) 3 4567-8900',
                    city='Apucarana-PR', event_id=self.event.pk)
        self.resp = self.client.post(r('events:subscription_new'), data)

    def test_post(self):
        """Valid POST should redirect to /eventos/inscricao/1/"""
        # 302 = redirect
        self.assertEqual(302, self.resp.status_code)
        self.assertRedirects(self.resp, r('events:subscription_detail', 1))
